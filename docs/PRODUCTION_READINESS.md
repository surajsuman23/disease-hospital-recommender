...
| Model consistency                        | Python/backend parity for 80 held-out DDXPlus symptom sets                                        |
...
| Clinical suitability                     | NOT READY: real disease labels from medically simulated DDXPlus; no clinical validation                                 |
...
The public release is suitable for demonstrating the software architecture with research data and a sourced hospital snapshot. It is not ready to accept patient records or make medical decisions. Replacing synthetic data, adding authentication or introducing persistence changes the risk and requires a separate design review.
